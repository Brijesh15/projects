...
DEFAULT_UDM_SIM="$SCRIPTPATH"/../../../../start_udmamf3gpp.sh
...
do
		...
		get_request_json_file=$(find . -maxdepth 1 -iname "*get*.json" ! -iname "*res*" | head -n 1)
		if [[ "$get_request_json_file" == "" ]]
		then
			echo "$tc_dir: NO GET request json"
		else
			#Any json file containg the string res in its name is expected result file. Result files are of 2 types: 1) post_exp_res 2) get_exp_res
			#If get file is present then its expected result get must also be present
			get_exp_resp_json_file=$(find . -maxdepth 1 -iname "*get_exp_res*.json" | head -n 1)
			if [[ "$get_exp_resp_json_file" == "" ]]
			then
				echo "$tc_dir: GET request present but NO GET RESPONSE. Skipping this test case..."
				continue
			fi
		fi
		...
		get_request_sh_file=$(find . -maxdepth 1 -iname "*get*.sh" ! -iname "*res*" | head -n 1)
		if [[ "$get_request_sh_file" != "" ]]
		then
			echo "$tc_dir: GET request SH FOUND"
		fi

		if [[ "$get_request_json_file" == "" && "$put_request_json_file" == "" && "$post_request_json_file" == ""  && "$post_request_sh_file" == "" && "$put_request_sh_file" == "" && "$get_request_sh_file" == "" ]]
		then
		...
		then
			...
			if [[ "$post_request_json_file" != "" ]]
			then
				curl -s --show-error -D post_resp_headers.txt -H "Content-Type: application/json" -H 'Accept: application/json; indent=4' 'http://127.0.0.1:9000/nudm-uecm/v1/imsi-311480012345674/registrations/amf-3gpp-access/' -d "$(cat "$post_request_json_file")" --request POST -o post_resp.json
		...
		then
			...
			#Send GET request and verify its results
			if [[ "$get_request_json_file" != "" ]]
			then
				curl -s --show-error -D get_resp_headers.txt -H "Content-Type: application/json" -H 'Accept: application/json; indent=4' "http://127.0.0.1:9000/nudm-uecm/v1/imsi-311480012345674/registrations/amf-3gpp-access/" --request GET -o get_resp.json
				jq . get_resp.json > get_resp_pretty.json.tmp
				diff -w get_resp_pretty.json.tmp "$get_exp_resp_json_file"
				if (( $? != 0 ))
				then
					echo "Ignoring $tc_dir: GET BODY FAILED..."
					[ "$udm_pgid" != "" ] && kill -s SIGKILL -- -"$udm_pgid" #Kill pgid of the sim that was run
					#sleep 1
					continue
				fi
		...
		if [ -f get_exp_hdr_res.txt ]
		then
			if [ ! -f get_resp_headers.txt ]
			then
				echo "Ignoring $tc_dir: GET RESPONSE NOT RECEIVED..."
				echo "$tc_dir: FAILED"
				[ "$udm_pgid" != "" ] && kill -s SIGKILL -- -"$udm_pgid" #Kill pgid of the sim that was run
				#sleep 1
				continue
			else
				#Compare GET response headers - Only Content-type and Response code are compared
				diff -w <(head -n 1 get_resp_headers.txt) <(head -n 1 get_exp_hdr_res.txt)
				if (( $? != 0 ))
				then
					echo "Ignoring $tc_dir: GET HEADER RESPONSE CODE FAILED..."
					[ "$udm_pgid" != "" ] && kill -s SIGKILL -- -"$udm_pgid" #Kill pgid of the sim that was run
					#sleep 1
					continue
				fi

				diff -w <(grep '^Content-Type' get_resp_headers.txt) <(grep Content-Type get_exp_hdr_res.txt)
				if (( $? != 0 ))
				then
					echo "Ignoring $tc_dir: GET HEADER CONTENT-TYPE FAILED..."
					[ "$udm_pgid" != "" ] && kill -s SIGKILL -- -"$udm_pgid" #Kill pgid of the sim that was run
					#sleep 1
					continue
				fi
