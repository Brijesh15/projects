...
DEFAULT_UDM_SIM="$SCRIPTPATH"/../../../../../../start_dummy_udrauthEvent.sh
...
do
		...
		then
			...
			if [[ "$post_request_json_file" != "" ]]
			then
				curl -s --show-error -D post_resp_headers.txt -H "Content-Type: application/json" -H 'Accept: application/json; indent=4' 'http://127.0.0.1:9000/nudm-ueau/v1/imsi-311480012345674/auth-events/' -d "$(cat "$post_request_json_file")" --request POST -o post_resp.json
