...
for tc_dir in $(find "$SCRIPTPATH"/tests/ -maxdepth 1 -mindepth 1 -type d)
do
	(
		if [[ "$test_dir_name" != "" ]] 
		then
		...
		udm_sim=$(find . -maxdepth 1 -iname "*udm*" ! -iname "*.json")
		if [[ "$udm_sim" == "" ]]
		then
		...
		then
			...
			if [[ "$post_request_json_file" != "" ]]
			then
				curl -s --show-error -D post_resp_headers.txt -H "Content-Type: application/json" -H 'Accept: application/json; indent=4' 'http://127.0.0.1:8000/imsi-311480012345674/security-information/generate-auth-data/' -d "$(cat "$post_request_json_file")" --request POST -o post_resp.json
