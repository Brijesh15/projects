...
#                return response
            ...
            json_res = {"success" : False, "message": "error occurred while getting company Qr list"}
            return HttpResponse(json.dumps(json_res), content_type="application/json")
...
#                    counter+=1
            up_obj = uploaddownload()
            if company_name == 'PhonePe':
                Res = up_obj.PhonePeUpload(db, confDict, company_id, filelist)
                return Res
            elif(company_name == 'Airtel'):
                pass
...
class uploaddownload:
    ...
    def PhonePeUpload(self, db, confDict, company_id, filelist):
        print("PhonePe")
        cmd = "select count(*) FROM information_schema.tables WHERE table_schema = 'company' AND table_name = 'PhonePe';"
        intry = db.get_raw_query(cmd)
        print("intry",intry[0][0])
        if not intry[0][0]:
            cmd = "CREATE TABLE PhonePe (id INT(11) NOT NULL AUTO_INCREMENT, company_id int, scanned_result varchar(50) , qrid VARCHAR(500), qrcode VARCHAR(500), lot_number varchar(20), created_at int(12), updated_at int(12), PRIMARY KEY (id));"
            db.insert_data(cmd)
        ...
        for csvfile in filelist:
            ...
            if len(checkColumn.split(',')) != int(confDict.get('PhonePe')):
                json_res = {"success" : False, "message": "Contents of uploaded csv file %s's format is not supported \
                            against this company"%(csvfile.name)}
                return HttpResponse(json.dumps(json_res), content_type="application/json")
            ...
            for column in csv.reader(io_string, delimiter=','):
                ...
                cmd = "select * from PhonePe where company_id=%s and qrcode='%s'"%(company_id, column[1])
                #print(cmd)
                intry = db.check_interies(cmd)
                #print(flag)
                if intry:
                    print("%s number qrcode in file %s already in the database against this company"%(counter, csvfile.name))
                    json_res = {"success" : False, "message": "%s number qrcode in file %s already in the database \
                                against this company"%(counter, csvfile.name)}
                    return HttpResponse(json.dumps(json_res), content_type="application/json")
                    #continue
                print("rfrf",column[2].lstrip('0'))
                cmd = "insert into PhonePe (company_id, scanned_result, qrid, qrcode, lot_number, created_at) values \
                (%s, 'not scanned','%s', '%s', '%s', '%s')"%(company_id, column[0], column[1], int(column[2].lstrip('0')), time.time())
                print('cmd %s'%cmd)
                db.insert_data(cmd)
        json_res = {"success" : True, "message": "file uploaded successfully"}
        return HttpResponse(json.dumps(json_res), content_type="application/json")
